...
def format_date_for_api(date_value: Optional[date]) -> Optional[str]:
    """Date formation for API RetailCRM"""
    if date_value:
...
def prepare_customer_data(customer: Dict[str, Any]) -> Dict[str, Any]:
    """Customer data preparation for RetailCRM"""
    return {
...
def prepare_order_data(order: Dict[str, Any]) -> Dict[str, Any]:
    """Order data preparation for RetailCRM"""
    return {
        "number": order.get("order_number", ""),
        "customer": {
            "id": order.get("customer_id", 0),
...
def prepare_payment_data(
    ...
    """Payment data preparation for RetailCRM"""
    payment = {"order": {"id": order_id}, "amount": amount, "type": payment_type}
